...
for speaker in $DATA_ORIG/* ; do
   ...
   for waves in $speaker/S*/wav_* ; do
      ...
      if  [ -d "$waves" ] ; then
         acoustics=true
         transcript="${waves/wav_*Mic/prompts}" #/wav_*Mic/prompts 表示把wav_*Mic替换成prompts
...
#  get the nomenclature
   ...
   ssn=$(basename $session)   # e.g. Session2
   tmp=${session#*torgo/}     # e.g. F03/Sesson2
   spk=${tmp%/Sess*}          # e.g. F03
   mic=${waves#*wav_}         # e.g. headMic
   echo "  $spk $ssn $mic" 
   gender=${spk:0:1}
   gender=${gender,,}         # e.g. f
   for doc in $session/prompts/* ; do
      ...
      utt="${doc%.txt}"
      utt=$(basename $utt)       # e.g. 1
      id="$spk-$ssn-$mic-$utt"   # e.g. F03-Session2-headMic-1
      line="$id ${line^^}"       # 将line转换成大写
      if [ -f $waves/$utt.wav ] ; then  #only files with all the associated info are written
         ...
         echo "$line" | tr -d '[.,?!:;"\r]' \
                | sed 's/.//g' \
                | sed 's/XRAY/X-RAY/g' \
                | awk '{$1=$1;print}' >> data/train/text.unsorted
   ...
   for doc in $session/prompts/* ; do
      ...
      if [ -f $waves/$utt.wav ] ; then   #only files with all the associated info are written
         ...
         echo "$line" | tr -d '[.,?!:;"\r]' \
                | sed 's/.//g' \
                | sed 's/XRAY/X-RAY/g' \
                | awk '{$1=$1;print}' >> data/test/text.unsorted
